...
import { clearPrincipalHeaders, getPrincipalHeaders, setPrincipalHeaders } from '@/api/client'
...
const session = ref<api.WorkbenchSession | null>(null)
const summary = ref<api.WorkbenchSummary | null>(null)
const releases = ref<Array<Record<string, any>>>([])
const audit = ref<Awaited<ReturnType<typeof api.getAuditTrail>> | null>(null)
const identityOpen = ref(false)
const releaseOpen = ref(false)
const auditOpen = ref(false)
const principalEnvelope = ref(JSON.stringify(getPrincipalHeaders(), null, 2))

const editor = reactive({ ontology_text: '', shapes_text: '', skos_text: '' })
const actor = computed(() => session.value ? `${session.value.roles[0] || 'principal'}:${session.value.subject}` : '未认证')
const createForm = reactive({ ontology_id: '', ontology_text: '', shapes_text: '', skos_text: '' })
...
const auditNodes = computed(() => [...(audit.value?.causal_chain.nodes ?? [])].reverse())
const can = (permission: keyof api.WorkbenchSession['permissions']) => Boolean(session.value?.permissions[permission])
...
async function refreshOverview() {
  const [nextSession, nextSummary, releaseResult] = await Promise.all([
    api.getWorkbenchSession(), api.getWorkbenchSummary(), api.listReleases(),
  ])
...
function showCreate() {
  Object.assign(createForm, { ontology_id: '', ontology_text: defaultOntology, shapes_text: defaultShapes, skos_text: defaultSkos })
...
async function createDraft() {
  ...
  await refreshDrafts(); await refreshOverview(); await openDraft(created.draft_id)
...
async function saveDraft() {
  ...
  saving.value = true
  try {
    await api.updateDraft(activeId.value, { ...editor })
    await openDraft(activeId.value); await refreshDrafts(); await refreshOverview()
...
async function validate() {
  if (!bundle.value) return
  const result = await api.validateDraft(activeId.value)
  await openDraft(activeId.value); await refreshDrafts(); await refreshOverview()
...
async function waive(finding: GovernanceFinding) {
  ...
  await api.waiveFinding(activeId.value, { finding_id: finding.finding_id, rationale, policy })
  await openDraft(activeId.value); await refreshOverview()
...
async function approve() {
  ...
  await api.approveDraft(activeId.value, { rationale, policies: ['policy:ontology-release-gate'] })
  await openDraft(activeId.value); await refreshDrafts(); await refreshOverview()
...
async function requestChanges() {
  ...
  await api.requestChanges(activeId.value, rationale)
  await openDraft(activeId.value); await refreshDrafts(); await refreshOverview()
...
async function publish() {
  ...
  const result = await api.publishDraft(activeId.value)
  await openDraft(activeId.value); await refreshDrafts(); await refreshOverview()
...
async function showAudit() {
  ...
  audit.value = await api.getAuditTrail(activeId.value)
  auditOpen.value = true
}

function savePrincipalEnvelope() {
  try {
    const parsed = JSON.parse(principalEnvelope.value)
    const required = ['x-aof-principal-subject', 'x-aof-principal-tenant', 'x-aof-principal-roles', 'x-aof-principal-timestamp', 'x-aof-principal-key-id', 'x-aof-principal-signature']
    if (!parsed || required.some((key) => !parsed[key])) throw new Error('签名 envelope 缺少必需字段')
    setPrincipalHeaders(parsed)
    identityOpen.value = false
    initialize()
  } catch (error) { ElMessage.error(error instanceof Error ? error.message : '无效的 principal envelope') }
}

function disconnectPrincipal() {
  clearPrincipalHeaders(); session.value = null; drafts.value = []; bundle.value = null
  principalEnvelope.value = '{}'; identityOpen.value = true
}
...
async function initialize() {
  loading.value = true
  try {
    await Promise.all([refreshOverview(), refreshDrafts()])
    if (drafts.value.length) await openDraft(activeId.value || drafts.value[0].draft_id)
  } catch {
  ...
  <div class="governance-page">
    <header class="hero">
      ...
      <div class="hero-actions">
        <button class="principal-chip" @click="identityOpen = true">
          <span>{{ session?.tenant_id || 'NO TENANT' }}</span>
          <strong>{{ actor }}</strong>
        </button>
        <el-button class="new-button" :disabled="!can('edit')" @click="showCreate">＋ 新建治理草稿</el-button>
      </div>
    </header>

    <section class="governance-pulse">
      <div><span>治理中草稿</span><strong>{{ summary?.draft_count ?? '—' }}</strong><small>{{ summary?.state_counts.conflict_review || 0 }} 个待冲突处置</small></div>
      <div><span>未解决违规</span><strong class="danger-number">{{ summary?.findings.unresolved ?? '—' }}</strong><small>{{ summary?.findings.waived || 0 }} 个已记录豁免</small></div>
      <div><span>不可变 Release</span><strong>{{ summary?.release_count ?? '—' }}</strong><button @click="releaseOpen = true">查看时间线 →</button></div>
      <div class="integrity-cell"><span>证据完整性</span><strong>{{ summary?.evidence_integrity.valid ? 'VERIFIED' : 'UNKNOWN' }}</strong><small>{{ summary?.evidence_integrity.decision_ledger.entries_checked || 0 }} 条决策已校验</small></div>
    </section>
      ...
      <main v-loading="loading" class="editor-shell">
        <template v-if="bundle">
          ...
          <div class="action-strip">
            <el-button :disabled="!can('edit') || !['draft', 'changes_requested'].includes(bundle.manifest.state)" @click="entityOpen = true">结构化添加</el-button>
            <el-button :loading="saving" :disabled="!can('edit') || !['draft', 'changes_requested'].includes(bundle.manifest.state)" @click="saveDraft">保存新修订</el-button>
            <el-button type="primary" :disabled="!can('validate') || !['draft', 'changes_requested'].includes(bundle.manifest.state)" @click="validate">运行治理门禁</el-button>
            <el-button @click="showImpact">影响预览</el-button>
            <el-button @click="showAudit">审计轨迹</el-button>
            <span class="action-spacer"></span>
            <el-button :disabled="!can('review') || !['validated', 'conflict_review'].includes(bundle.manifest.state)" @click="requestChanges">请求修改</el-button>
            <el-button type="warning" :disabled="!can('review') || !['validated', 'conflict_review'].includes(bundle.manifest.state)" @click="approve">审批</el-button>
            <el-button class="publish-button" :disabled="!can('publish') || bundle.manifest.state !== 'approved'" @click="publish">发布不可变版本</el-button>
          </div>
            ...
            <el-table v-if="latestReview?.findings.length" :data="latestReview.findings" size="small" class="finding-table">
              ...
              <el-table-column label="处置" width="110">
                <template #default="{ row }">
                  ...
                  <el-button v-else link type="warning" :disabled="!can('waive')" @click="waive(row)">记录豁免</el-button>
                </template>
              </el-table-column>
            </el-table>
    ...
    <el-dialog v-model="createOpen" title="创建受治理本体草稿" width="78%" top="4vh">
      <el-form label-position="top">
        <el-row :gutter="16"><el-col :span="12"><el-form-item label="本体 ID"><el-input v-model="createForm.ontology_id" placeholder="customer-domain" /></el-form-item></el-col><el-col :span="12"><el-form-item label="签名创建者"><el-input :model-value="actor" disabled /></el-form-item></el-col></el-row>
    ...
    <el-drawer v-model="releaseOpen" title="不可变 Release 时间线" size="48%">
      <div v-if="releases.length" class="release-timeline">
        <article v-for="item in releases" :key="item.ontology_version">
          <i></i><div><span>{{ item.ontology_id }}</span><h4>{{ item.ontology_version }}</h4><p>{{ item.published_at }} · {{ item.published_by }}</p><small>+{{ item.change_set?.added?.length || 0 }} / −{{ item.change_set?.removed?.length || 0 }} triples · {{ item.integrity?.hash?.slice(0, 18) }}…</small></div>
        </article>
      </div>
      <div v-else class="select-empty">尚无已发布版本。</div>
    </el-drawer>

    <el-drawer v-model="auditOpen" title="决策级审计轨迹" size="56%">
      <div v-if="audit" class="audit-head"><span>CHAIN INTEGRITY</span><strong>{{ audit.integrity.valid ? 'VALID' : 'INVALID' }}</strong><small>{{ audit.decision_id || '尚无治理决策' }}</small></div>
      <div class="audit-chain">
        <article v-for="(node, index) in auditNodes" :key="node.decision.id">
          <b>{{ String(index + 1).padStart(2, '0') }}</b><div><span>{{ node.decision.decision_type }}</span><h4>{{ node.decision.conclusion }}</h4><p>{{ node.decision.rationale }}</p><small>{{ node.decision.agent_id }} · {{ node.decision.timestamp }}</small></div>
        </article>
      </div>
    </el-drawer>

    <el-dialog v-model="identityOpen" title="连接企业身份网关" width="620px" :close-on-click-modal="Boolean(session)">
      <div class="identity-note"><b>浏览器不持有签名密钥。</b><p>粘贴由企业身份网关签发的短时 principal header envelope。AOF 会验证签名、时效、租户和角色；表单中的自报身份不会被信任。</p></div>
      <el-input v-model="principalEnvelope" type="textarea" :rows="11" spellcheck="false" placeholder='{ "x-aof-principal-subject": "..." }' />
      <template #footer><el-button v-if="session" type="danger" plain @click="disconnectPrincipal">断开身份</el-button><el-button type="primary" @click="savePrincipalEnvelope">验证并连接</el-button></template>
    </el-dialog>
  </div>
</template>
...
.hero .eyebrow { color:var(--acid); }.hero h1{font:700 35px/1.1 Georgia,"Songti SC",serif;margin:8px 0}.hero p{margin:0;color:#aebdb8;font-size:13px}.hero-actions{display:flex;gap:10px;z-index:1}.new-button{background:var(--acid);color:var(--ink);border-color:var(--acid);font-weight:700}.principal-chip{min-width:195px;text-align:left;border:1px solid #496159;background:#1a332d;color:white;padding:7px 12px;cursor:pointer}.principal-chip span{display:block;font:8px ui-monospace,monospace;letter-spacing:.13em;color:var(--acid)}.principal-chip strong{font-size:11px;font-weight:500}
.governance-pulse{display:grid;grid-template-columns:repeat(4,1fr);background:#fff;border-bottom:1px solid #cbc7bc}.governance-pulse>div{min-height:92px;padding:16px 20px;border-right:1px solid #ddd8ce;display:grid;grid-template-columns:1fr auto;align-items:center}.governance-pulse span{font-size:10px;color:#727b77;letter-spacing:.05em}.governance-pulse strong{font:700 27px Georgia,serif}.governance-pulse small{grid-column:1/-1;font:9px ui-monospace,monospace;color:#8a918e}.governance-pulse button{grid-column:1/-1;border:0;background:none;text-align:left;padding:0;color:#4e6c61;font-size:10px;cursor:pointer}.governance-pulse .danger-number{color:#b2402f}.governance-pulse .integrity-cell{background:#152a25;color:white}.integrity-cell span,.integrity-cell small{color:#a9bbb5}.integrity-cell strong{color:var(--acid);font:700 15px ui-monospace,monospace;letter-spacing:.08em}
...
.release-timeline article,.audit-chain article{display:grid;grid-template-columns:28px 1fr;gap:16px;position:relative;padding:0 0 25px}.release-timeline article::before,.audit-chain article::before{content:"";position:absolute;left:8px;top:13px;bottom:0;border-left:1px solid #c9cec9}.release-timeline article:last-child::before,.audit-chain article:last-child::before{display:none}.release-timeline i{width:17px;height:17px;border-radius:50%;background:var(--acid);border:5px solid #203c34;z-index:1}.release-timeline span,.audit-chain span{font:9px ui-monospace,monospace;color:#6d7974;text-transform:uppercase}.release-timeline h4,.audit-chain h4{margin:3px 0;font:700 17px Georgia,serif}.release-timeline p,.release-timeline small,.audit-chain p,.audit-chain small{margin:3px 0;color:#76807c;font-size:11px}.audit-head{background:#152a25;color:white;padding:18px;margin-bottom:22px;display:grid;grid-template-columns:1fr auto}.audit-head span{font:9px ui-monospace,monospace;color:#9aada7}.audit-head strong{color:var(--acid);font:700 12px ui-monospace,monospace}.audit-head small{grid-column:1/-1;margin-top:8px;color:#9aada7}.audit-chain article>b{display:grid;place-items:center;width:24px;height:24px;border-radius:50%;background:#e7eadf;font:9px ui-monospace,monospace;z-index:1}.identity-note{border-left:4px solid var(--acid);background:#eff2e8;padding:13px 16px;margin-bottom:14px}.identity-note b{font-size:13px}.identity-note p{font-size:11px;line-height:1.6;margin:5px 0 0;color:#66706c}
@media(max-width:1000px){.workbench{grid-template-columns:1fr}.draft-rail{display:flex;overflow:auto;border-right:0;border-bottom:1px solid #d7d2c7}.rail-title{display:none}.draft-card{min-width:170px}.hero{align-items:flex-start;gap:20px}.hero-actions{flex-direction:column}.process-node{padding:10px}.process-node span{display:none}.governance-pulse{grid-template-columns:repeat(2,1fr)}}
</style>
