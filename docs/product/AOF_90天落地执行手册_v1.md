# AOF 90天落地执行手册 v1（历史版本）

> **已由 [AOF 运行时平台定位与财务控制平面 v2](AOF_运行时平台定位与财务控制平面_v2.md) 取代其首个场景和价值主张。**
>
> 本文中的“Text-to-SQL + 指标口径治理”可作为基础控制能力或子工作包，但不再是 AOF 的主战场定义。后续 90 天计划应以一个财务例外闭环的行动、审批、对账与价值归因为主线。
