...
- 发布版本绑定的 Agentic 查询与业务 QueryContract 计划：SQL、RDF、证据检索、分析 Skill、规则及待审批动作。
- 财务单域本地 E2E：收入/预算/回款、合同条款、异常规则、独立审批、持久工单和不确定结果对账状态；使用合成样本。
- 修复默认自然语言查询适配、重复执行、回放污染记忆、执行前版本校验和 action 审批租户检查。
- 同步受治理架构和 API 文档，OpenAPI 从应用生成并在 CI 检查漂移。
