...
def test_okf_index_reads_progressive_entry(tmp_path):
    ...
    server = build_server()
    result = asyncio.run(_tool(server, "aof_okf_index").handler({"bundle_dir": str(bundle)}))
...
def test_okf_index_missing_bundle(tmp_path):
    server = build_server()
    result = asyncio.run(_tool(server, "aof_okf_index").handler({"bundle_dir": str(tmp_path)}))
...
def test_okf_get_concept_reads_full(tmp_path):
    bundle = _write_bundle(tmp_path)
    server = build_server()
    result = asyncio.run(_tool(server, "aof_okf_get_concept").handler(
...
def test_okf_search_concepts_by_type_and_query(tmp_path):
    ...
    by_type = asyncio.run(_tool(server, "aof_okf_search_concepts").handler(
        {"bundle_dir": str(bundle), "type": "person"}
    ))
    assert by_type["count"] == 1
    assert by_type["results"][0]["title"] == "Alice"

    by_query = asyncio.run(_tool(server, "aof_okf_search_concepts").handler(
...
def test_okf_search_concepts_no_match(tmp_path):
    bundle = _write_bundle(tmp_path)
    server = build_server()
    result = asyncio.run(_tool(server, "aof_okf_search_concepts").handler(
...
def test_okf_lint_report(tmp_path):
    bundle = _write_bundle(tmp_path)
    server = build_server()
    report = asyncio.run(_tool(server, "aof_okf_lint").handler({"bundle_dir": str(bundle)}))
...
def test_default_bundle_dir_from_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("mcp_server.os.environ", {**os.environ, "AOF_OKF_DIR": str(tmp_path)})
