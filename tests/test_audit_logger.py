...
class TestAuditLogger:
    ...
    async def test_log_event(self, logger):
        ...
        result = await logger.log_event(event)
    ...
    @pytest.mark.asyncio
    async def test_log_with_masking(self, logger):
        """测试带脱敏的记录"""
        event = AuditEvent(
            user_id="user_123",
            action="auth:login",
    ...
    @pytest.mark.asyncio
    async def test_immediate_flush(self, logger):
        """测试立即写入"""
        event = AuditEvent(
            user_id="user_123",
        ...
        # 但由于没有配置后端，这里只检查不报错
        result = await logger.log_event(event, immediate=True)
        assert result is True
    
    @pytest.mark.asyncio
    async def test_logger_stats(self, logger):
        """测试统计信息"""
        # 记录一些事件
        for i in range(5):
            await logger.log(
    ...
    def test_logger_start_stop(self, logger):
        """测试启动和停止"""
        import asyncio
