...
from bridge.training_data.generators.base import GeneratorBase
...
class TrainingDataExporter:
    ...
    def _resolve_generators(
        names: list[str], raw_sources: Optional[list[str]] = None
    ) -> list[GeneratorBase]:
        """将生成器名称解析为实例."""
        mapping: dict[str, Callable[[], GeneratorBase]] = {
            "sft": lambda: SFTGenerator(),
            "rag_eval": lambda: RAGEvalGenerator(),
            "agent_tool": lambda: AgentToolGenerator(),
        }
        instances: list[GeneratorBase] = []
        for name in names:
            if name == "raw":
                raw_generator = RawTrajectoryGenerator()
                if raw_sources:
                    raw_generator = raw_generator.with_sources(raw_sources)
                instances.append(raw_generator)
                continue
