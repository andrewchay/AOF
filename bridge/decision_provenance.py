...
class DecisionProvenanceStore:
    ...
    def record(
        ...
        *,
        ...
        if not all(isinstance(value, str) and value.strip() for value in (agent_id, decision_type, conclusion, rationale)):
            raise DecisionProvenanceError("agent_id, decision_type, conclusion, and rationale must be non-empty strings")
        ...
        if unknown:
            raise DecisionProvenanceError(f"unknown parent decision(s): {', '.join(unknown)}")
        refs = [item if isinstance(item, EvidenceRef) else EvidenceRef(**item) for item in (evidence or [])]
        decision = DecisionRecord(
            id=record_id, recorded_at=_now(), agent_id=agent_id, decision_type=decision_type,
            conclusion=conclusion, rationale=rationale, status=status, tenant_id=tenant_id,
            session_id=session_id, evidence=refs, parent_decision_ids=parents,
            output_entities=list(output_entities or []), tags=sorted(set(tags or [])),
            policies=sorted(set(policies or [])), metadata=metadata or {},
        )
        previous_hash = entries[-1]["integrity"]["hash"] if entries else None
        payload = {"decision": decision.to_dict(), "previous_hash": previous_hash}
        entry = {**payload, "integrity": {"algorithm": "sha256", "hash": _hash(payload)}}
    ...
    def get(self, decision_id: str) -> dict[str, Any] | None:
        return next((entry for entry in self._entries() if entry["decision"]["id"] == decision_id), None)

    def causal_chain(self, decision_id: str, direction: str = "ancestors", max_depth: int = 8) -> dict[str, Any]:
        ...
        while queue:
            ...
            if depth >= max_depth:
                continue
            neighbors = by_id[current]["decision"].get("parent_decision_ids", []) if direction == "ancestors" else children.get(current, [])
            for other in sorted(neighbors):
                edges.append({"from": current if direction == "descendants" else other, "to": other if direction == "descendants" else current, "type": "wasInformedBy"})
                if other not in visited and other in by_id:
                    visited.add(other); nodes.append(by_id[other]); queue.append((other, depth + 1))
        return {"root_decision_id": decision_id, "direction": direction, "nodes": nodes, "edges": edges}

    def find_precedents(self, decision_type: str, tags: Iterable[str] | None = None, tenant_id: str | None = None, limit: int = 20) -> list[dict[str, Any]]:
        wanted = set(tags or [])
        matches = []
        for entry in self._entries():
            decision = entry["decision"]
            if decision["decision_type"] != decision_type or (tenant_id and decision.get("tenant_id") != tenant_id):
                continue
            overlap = wanted.intersection(decision.get("tags", []))
            score = len(overlap) / len(wanted) if wanted else 1.0
            matches.append({"decision": entry, "similarity": score, "matching_tags": sorted(overlap)})
        return sorted(
            matches,
            key=lambda item: (item["similarity"], item["decision"]["decision"]["recorded_at"]),
            reverse=True,
        )[:limit]

    def impact(self, decision_id: str, max_depth: int = 8) -> dict[str, Any]:
        chain = self.causal_chain(decision_id, direction="descendants", max_depth=max_depth)
        outputs = []
        for entry in chain["nodes"]:
            for entity in entry["decision"].get("output_entities", []):
                outputs.append({"decision_id": entry["decision"]["id"], "entity": entity})
        return {**chain, "affected_entities": outputs, "affected_decision_count": max(0, len(chain["nodes"]) - 1)}
    ...
    def audit_trail(self, decision_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "integrity": integrity,
            "compliance": {
                "policy_references": sorted({policy for node in chain["nodes"] for policy in node["decision"].get("policies", [])}),
                "evidence_complete": all(node["decision"].get("evidence") for node in chain["nodes"]),
                "rationale_complete": all(node["decision"].get("rationale") for node in chain["nodes"]),
            },
        }
    ...
    def verify_integrity(self) -> dict[str, Any]:
        ...
        for index, entry in enumerate(self._entries(), start=1):
            payload = {"decision": entry.get("decision"), "previous_hash": entry.get("previous_hash")}
            if entry.get("previous_hash") != previous_hash or entry.get("integrity", {}).get("hash") != _hash(payload):
                return {"valid": False, "entries_checked": index, "failed_decision_id": entry.get("decision", {}).get("id")}
            previous_hash = entry["integrity"]["hash"]
        return {"valid": True, "entries_checked": index if 'index' in locals() else 0, "head_hash": previous_hash}
    ...
    def _entries(self) -> list[dict[str, Any]]:
        ...
        for line_number, line in enumerate(self.path.read_text(encoding="utf-8").splitlines(), start=1):
            ...
            try:
                ...
                raise DecisionProvenanceError(f"invalid provenance ledger at line {line_number}") from exc
        return entries
