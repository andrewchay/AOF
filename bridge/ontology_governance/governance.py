...
SUPPORTED_CONSTRAINTS = {
    ...
    SH.minInclusive, SH.maxInclusive, SH.minLength, SH.maxLength, SH.pattern, SH["in"],
}
...
class ShaclCoreValidator:
    ...
    def validate(self, data_graph: Graph, shapes_graph: Graph) -> dict[str, Any]:
        ...
        node_shapes = sorted(set(shapes_graph.subjects(RDF.type, SH.NodeShape)), key=str)
        for shape in node_shapes:
            targets = self._targets(data_graph, shapes_graph, shape)
            for predicate in shapes_graph.predicates(shape, None):
                allowed = {RDF.type, SH.targetNode, SH.targetClass, SH.targetSubjectsOf, SH.targetObjectsOf, SH.property, SH.severity, SH.message, SH.name, SH.order, SH["class"], SH.nodeKind}
                if str(predicate).startswith(str(SH)) and predicate not in allowed:
                    unsupported.append(self._unsupported(shape, shape, f"unsupported SHACL node constraint: {predicate}"))
            ...
            for focus in targets:
                if required_node_class and not self._is_instance_of(data_graph, focus, required_node_class):
                    findings.append(self._node_finding(shape, focus, "ClassConstraintComponent", f"focus node must be an instance of {required_node_class}"))
                if required_node_kind and not self._matches_node_kind(focus, required_node_kind):
                    findings.append(self._node_finding(shape, focus, "NodeKindConstraintComponent", f"focus node must be {self._local(required_node_kind)}"))
            for prop_shape in shapes_graph.objects(shape, SH.property):
                path = shapes_graph.value(prop_shape, SH.path)
                if not isinstance(path, URIRef):
                    unsupported.append(self._unsupported(shape, prop_shape, "only direct URI sh:path is supported"))
                    continue
                for predicate in shapes_graph.predicates(prop_shape, None):
                    if predicate not in SUPPORTED_CONSTRAINTS | {SH.path, RDF.type, SH.severity, SH.message, SH.name, SH.order}:
                        if str(predicate).startswith(str(SH)):
                            unsupported.append(self._unsupported(shape, prop_shape, f"unsupported SHACL constraint: {predicate}"))
                for focus in targets:
                    values = list(data_graph.objects(focus, path))
                    findings.extend(self._validate_property(shapes_graph, shape, prop_shape, focus, path, values, data_graph))
    ...
    def _targets(self, data: Graph, shapes: Graph, shape: Any) -> list[Any]:
        ...
        for target_class in shapes.objects(shape, SH.targetClass):
            targets.update(subject for subject in data.subjects(RDF.type, None) if self._is_instance_of(data, subject, target_class))
    ...
    def _validate_property(self, shapes: Graph, shape: Any, prop_shape: Any, focus: Any, path: URIRef, values: list[Any], data: Graph) -> list[dict[str, Any]]:
        findings = []
        severity = self._local(shapes.value(prop_shape, SH.severity) or shapes.value(shape, SH.severity) or SH.Violation)
        message = str(shapes.value(prop_shape, SH.message) or "")

        def add(component: str, value: Any = None, detail: str = "") -> None:
            finding = {
                "type": "shacl_violation", "severity": severity, "source_shape": str(shape),
                "property_shape": str(prop_shape), "focus_node": str(focus), "path": str(path),
                "value": None if value is None else str(value), "constraint_component": component,
        ...
        if min_count is not None and len(values) < int(min_count):
            add("MinCountConstraintComponent", detail=f"expected at least {int(min_count)} value(s), found {len(values)}")
        if max_count is not None and len(values) > int(max_count):
            add("MaxCountConstraintComponent", detail=f"expected at most {int(max_count)} value(s), found {len(values)}")
        ...
        for value in values:
            if datatype and (not isinstance(value, Literal) or (value.datatype != datatype and not (datatype == XSD.string and value.datatype is None))):
                add("DatatypeConstraintComponent", value, f"value must have datatype {datatype}")
            if required_class and not self._is_instance_of(data, value, required_class):
                add("ClassConstraintComponent", value, f"value must be an instance of {required_class}")
            if node_kind and not self._matches_node_kind(value, node_kind):
                add("NodeKindConstraintComponent", value, f"value must be {self._local(node_kind)}")
    ...
    def _validate_literal_ranges(self, shapes: Graph, prop_shape: Any, value: Any, add) -> None:
        ...
        for predicate, operator, component in (
            ...
            if bound is not None:
                try:
                    ...
                    valid = False
                if not valid:
                    add(component, value, f"value violates {self._local(predicate)} {bound}")
        text = str(value)
        for predicate, operator, component in (
            ...
            if bound is not None and not operator(len(text), int(bound)):
                add(component, value, f"string length violates {self._local(predicate)} {bound}")
        pattern = shapes.value(prop_shape, SH.pattern)
        if pattern is not None:
            import re
            if re.search(str(pattern), text) is None:
                add("PatternConstraintComponent", value, f"value does not match {pattern}")

    def _closed_world_conflicts(self, data: Graph) -> list[dict[str, Any]]:
        findings = []
        disjoint = {(left, right) for left, right in data.subject_objects(OWL.disjointWith)}
        for left, right in disjoint:
            for entity in set(data.subjects(RDF.type, left)).intersection(data.subjects(RDF.type, right)):
                item = {
                    "type": "owl_conflict", "severity": "Violation", "focus_node": str(entity),
                    "constraint_component": "DisjointClassesConflict",
                    "message": f"entity is typed as disjoint classes {left} and {right}",
                    "left_class": str(left), "right_class": str(right),
                }
        ...
        for prop in data.subjects(RDF.type, OWL.FunctionalProperty):
            for subject in set(data.subjects(prop, None)):
                ...
                if len(values) > 1:
                    item = {
                        "type": "owl_conflict", "severity": "Violation", "focus_node": str(subject),
                        "path": str(prop), "constraint_component": "FunctionalPropertyConflict",
    ...
    def _matches_node_kind(value: Any, node_kind: Any) -> bool:
        return {
            SH.IRI: isinstance(value, URIRef), SH.BlankNode: isinstance(value, BNode),
            SH.Literal: isinstance(value, Literal), SH.BlankNodeOrIRI: isinstance(value, (BNode, URIRef)),
            SH.BlankNodeOrLiteral: isinstance(value, (BNode, Literal)), SH.IRIOrLiteral: isinstance(value, (URIRef, Literal)),
        }.get(node_kind, False)

    @staticmethod
    def _is_instance_of(graph: Graph, entity: Any, required_class: Any) -> bool:
        frontier = list(graph.objects(entity, RDF.type)); seen = set()
        while frontier:
    ...
    def _unsupported(self, shape: Any, prop_shape: Any, message: str) -> dict[str, Any]:
        return {"type": "unsupported_shacl_constraint", "severity": "Violation", "source_shape": str(shape), "property_shape": str(prop_shape), "message": message}

    def _node_finding(self, shape: Any, focus: Any, component: str, message: str) -> dict[str, Any]:
        finding = {"type": "shacl_violation", "severity": "Violation", "source_shape": str(shape), "focus_node": str(focus), "constraint_component": component, "message": message}
        finding["finding_id"] = f"finding:{_digest(finding)[:20]}"
        return finding
...
class OntologyGovernanceService:
    ...
    def __init__(self, root: str | Path, decision_store: DecisionProvenanceStore | None = None) -> None:
        self.root = Path(root)
    ...
    def create_draft(self, *, ontology_id: str, created_by: str, ontology_text: str, shapes_text: str, skos_text: str = "", base_version: str | None = None) -> dict[str, Any]:
        ...
        draft_dir = self._draft_dir(draft_id); draft_dir.mkdir(parents=True)
        ...
        manifest = {"draft_id": draft_id, "ontology_id": ontology_id, "created_by": created_by, "created_at": _now(), "base_version": base_version, "state": "draft", "revision": 1}
        self._write_json(draft_dir / "manifest.json", manifest)
        return manifest

    def update_draft(self, draft_id: str, *, actor: str, ontology_text: str | None = None, shapes_text: str | None = None, skos_text: str | None = None) -> dict[str, Any]:
        manifest = self.get_draft(draft_id)
        if manifest["state"] not in {"draft", "changes_requested"}:
            raise OntologyGovernanceError(f"draft cannot be edited in state {manifest['state']}")
        draft_dir = self._draft_dir(draft_id)
        for name, text in (("ontology.ttl", ontology_text), ("shapes.ttl", shapes_text), ("skos.ttl", skos_text)):
            if text is not None:
                if name != "skos.ttl" or text.strip():
                    _parse_rdf(text, name=name)
                (draft_dir / name).write_text(text, encoding="utf-8")
        manifest.update({"state": "draft", "revision": manifest["revision"] + 1, "updated_at": _now(), "updated_by": actor})
        self._write_json(draft_dir / "manifest.json", manifest)
        return manifest

    def validate_draft(self, draft_id: str, *, actor: str) -> dict[str, Any]:
        manifest = self.get_draft(draft_id); draft_dir = self._draft_dir(draft_id)
        data = _parse_rdf((draft_dir / "ontology.ttl").read_text(encoding="utf-8"), name="ontology")
        skos_text = (draft_dir / "skos.ttl").read_text(encoding="utf-8")
        if skos_text.strip():
            skos = _parse_rdf(skos_text, name="skos")
            for triple in skos: data.add(triple)
        shapes = _parse_rdf((draft_dir / "shapes.ttl").read_text(encoding="utf-8"), name="shapes")
        ...
        result["summary"]["violations"] = sum(1 for item in result["findings"] if item["severity"] == "Violation")
        result["conforms"] = result["conforms"] and not result["findings"]
        result.update({"review_id": f"review:{uuid.uuid4()}", "draft_id": draft_id, "revision": manifest["revision"], "actor": actor, "recorded_at": _now()})
        ...
        parent_ids = [manifest["changes_request_decision_id"]] if manifest.get("changes_request_decision_id") else []
        validation_decision = self.decision_store.record(
            agent_id=actor, decision_type="ontology_validation",
            conclusion="conforms" if result["conforms"] else f"blocked_by_{len(result['findings'])}_finding(s)",
            ...
            evidence=[{"id": f"{draft_id}:{name}", "type": "ontology_input", "content_hash": digest} for name, digest in input_hashes.items()],
            policies=["policy:ontology-release-gate"], tags=["ontology", "validation"],
            output_entities=[{"id": result["review_id"], "type": "governance_review", "content_hash": result["integrity"]["hash"], "conforms": result["conforms"]}],
            metadata={"finding_ids": [item["finding_id"] for item in result["findings"]], "summary": result["summary"]},
        )
        manifest.update({
            "state": "validated" if result["conforms"] else "conflict_review",
            "latest_review_id": result["review_id"], "validated_at": _now(),
    ...
    def waive_finding(self, draft_id: str, *, finding_id: str, actor: str, rationale: str, policy: str, expires_at: str | None = None) -> dict[str, Any]:
        ...
        if finding_id not in {item["finding_id"] for item in review["findings"]}:
            raise OntologyGovernanceError(f"finding not found in latest review: {finding_id}")
        waiver = {"waiver_id": f"waiver:{uuid.uuid4()}", "finding_id": finding_id, "draft_id": draft_id, "review_id": review["review_id"], "actor": actor, "rationale": rationale, "policy": policy, "expires_at": expires_at, "recorded_at": _now()}
        self._chain_append(self._draft_dir(draft_id) / "waivers.jsonl", waiver)
        manifest = self.get_draft(draft_id)
        decision = self.decision_store.record(
            agent_id=actor, decision_type="ontology_finding_waiver", conclusion=f"waived {finding_id}",
            rationale=rationale, parent_decision_ids=[manifest["validation_decision_id"]],
            evidence=[{"id": review["review_id"], "type": "governance_review", "content_hash": review["integrity"]["hash"]}],
            policies=[policy], tags=["ontology", "waiver"],
            output_entities=[{"id": waiver["waiver_id"], "type": "governance_waiver", "content_hash": waiver["integrity"]["hash"]}],
            metadata={"finding_id": finding_id, "expires_at": expires_at},
        )
        manifest.setdefault("waiver_decision_ids", []).append(decision["decision"]["id"])
        self._write_json(self._draft_dir(draft_id) / "manifest.json", manifest)
        waiver["decision_id"] = decision["decision"]["id"]
        return waiver

    def request_changes(self, draft_id: str, *, reviewer: str, rationale: str) -> dict[str, Any]:
        manifest = self.get_draft(draft_id)
        if manifest["state"] not in {"validated", "conflict_review"}:
            raise OntologyGovernanceError(f"changes cannot be requested in state {manifest['state']}")
        review = self._latest_review(draft_id)
        decision = self.decision_store.record(
            agent_id=reviewer, decision_type="ontology_changes_requested", conclusion="changes_requested",
            rationale=rationale, parent_decision_ids=[manifest["validation_decision_id"]], evidence=[{"id": review["review_id"], "type": "shacl_review", "content_hash": review["integrity"]["hash"]}],
            policies=["policy:ontology-release-gate"], tags=["ontology", "review"],
            output_entities=[{"id": draft_id, "type": "ontology_draft", "revision": manifest["revision"]}],
        )
        action = {"action_id": f"action:{uuid.uuid4()}", "type": "changes_requested", "draft_id": draft_id, "revision": manifest["revision"], "review_id": review["review_id"], "reviewer": reviewer, "rationale": rationale, "decision_id": decision["decision"]["id"], "recorded_at": _now()}
        self._chain_append(self._draft_dir(draft_id) / "actions.jsonl", action)
        manifest.update({"state": "changes_requested", "changes_requested_at": _now(), "changes_requested_by": reviewer, "changes_request_decision_id": decision["decision"]["id"]})
        self._write_json(self._draft_dir(draft_id) / "manifest.json", manifest)
        return {"manifest": manifest, "action": action, "decision": decision}

    def approve(self, draft_id: str, *, approver: str, rationale: str, policies: Iterable[str] = ()) -> dict[str, Any]:
        manifest = self.get_draft(draft_id); review = self._latest_review(draft_id)
        review_integrity = self._verify_chain(self._draft_dir(draft_id) / "reviews.jsonl")
        waiver_integrity = self._verify_chain(self._draft_dir(draft_id) / "waivers.jsonl")
        if not review_integrity["valid"] or not waiver_integrity["valid"]:
            raise OntologyGovernanceError("approval blocked because review or waiver evidence integrity is invalid")
        if review["revision"] != manifest["revision"]:
            raise OntologyGovernanceError("latest validation does not cover the current draft revision")
        waived = {item["finding_id"] for item in self._read_jsonl(self._draft_dir(draft_id) / "waivers.jsonl")}
        unresolved = [item for item in review["findings"] if item["severity"] == "Violation" and item["finding_id"] not in waived]
        if unresolved:
            raise OntologyGovernanceError(f"approval blocked by {len(unresolved)} unresolved violation(s)")
        evidence = [{"id": review["review_id"], "type": "shacl_review", "content_hash": review["integrity"]["hash"]}]
        for waiver in self._read_jsonl(self._draft_dir(draft_id) / "waivers.jsonl"):
            evidence.append({"id": waiver["waiver_id"], "type": "governance_waiver", "content_hash": waiver["integrity"]["hash"]})
        decision = self.decision_store.record(
            agent_id=approver, decision_type="ontology_release_approval", conclusion="approved",
            rationale=rationale, evidence=evidence,
            parent_decision_ids=[manifest["validation_decision_id"], *manifest.get("waiver_decision_ids", [])],
            policies=list(policies), tags=["ontology", "approval"],
            output_entities=[{"id": draft_id, "type": "ontology_draft", "revision": manifest["revision"]}],
        )
        manifest.update({"state": "approved", "approved_at": _now(), "approved_by": approver, "approval_decision_id": decision["decision"]["id"]})
        self._write_json(self._draft_dir(draft_id) / "manifest.json", manifest)
    ...
    def publish(self, draft_id: str, *, actor: str) -> dict[str, Any]:
        ...
        draft_dir = self._draft_dir(draft_id)
        contents = {name: (draft_dir / name).read_text(encoding="utf-8") for name in ("ontology.ttl", "shapes.ttl", "skos.ttl")}
        content_hashes = {name: hashlib.sha256(text.encode("utf-8")).hexdigest() for name, text in contents.items()}
        version = f"v-{_digest(content_hashes)[:12]}"
        release_dir = self.root / "published" / manifest["ontology_id"] / version
        if release_dir.exists():
            raise OntologyGovernanceError(f"ontology version already published: {version}")
        release_dir.mkdir(parents=True)
        for name, text in contents.items(): (release_dir / name).write_text(text, encoding="utf-8")
        previous = self._latest_release(manifest["ontology_id"])
        changes = self._diff_release(previous, contents)
        release = {
            "ontology_id": manifest["ontology_id"], "ontology_version": version,
            "shape_version": f"shapes-{content_hashes['shapes.ttl'][:12]}",
            "skos_version": f"skos-{content_hashes['skos.ttl'][:12]}",
            "draft_id": draft_id, "revision": manifest["revision"], "published_at": _now(), "published_by": actor,
            "approval_decision_id": manifest["approval_decision_id"], "content_hashes": content_hashes,
            "previous_version": previous["ontology_version"] if previous else None, "change_set": changes,
        }
        release["integrity"] = {"algorithm": "sha256", "hash": _digest(release)}
        self._write_json(release_dir / "manifest.json", release)
        decision = self.decision_store.record(
            agent_id=actor, decision_type="ontology_publish", conclusion=f"published {version}",
            ...
            policies=["policy:ontology-release-gate"], tags=["ontology", "publish"],
            output_entities=[{"id": f"ontology:{manifest['ontology_id']}:{version}", "type": "ontology_release", "content_hash": release["integrity"]["hash"]}],
        )
        release["publish_decision_id"] = decision["decision"]["id"]
        self._write_json(release_dir / "manifest.json", release)
        manifest.update({"state": "published", "published_version": version, "publish_decision_id": decision["decision"]["id"]})
        self._write_json(draft_dir / "manifest.json", manifest)
    ...
    def impact_preview(self, draft_id: str) -> dict[str, Any]:
        ...
        previous = self._latest_release(manifest["ontology_id"])
        changes = self._diff_release(previous, contents)
        changed_terms = sorted({item[2] for group in ("added", "removed") for item in changes[group] if item[1].endswith(("subClassOf", "type", "broader", "narrower", "exactMatch", "prefLabel"))})
        return {"draft_id": draft_id, "base_version": previous["ontology_version"] if previous else None, "change_set": changes, "potentially_affected_terms": changed_terms, "requires_revalidation": bool(changes["added"] or changes["removed"])}

    def get_draft(self, draft_id: str) -> dict[str, Any]:
        path = self._draft_dir(draft_id) / "manifest.json"
        if not path.exists(): raise OntologyGovernanceError(f"draft not found: {draft_id}")
        return json.loads(path.read_text(encoding="utf-8"))

    def list_drafts(self) -> list[dict[str, Any]]:
        return [json.loads(path.read_text(encoding="utf-8")) for path in sorted((self.root / "drafts").glob("*/manifest.json"))]
    ...
    def get_release(self, ontology_id: str, version: str) -> dict[str, Any]:
        ...
        if not manifest_path.exists():
            raise OntologyGovernanceError(f"ontology release not found: {ontology_id}/{version}")
        return {
    ...
    def list_releases(self, ontology_id: str | None = None) -> list[dict[str, Any]]:
        ...
        paths = base.glob(f"{ontology_id}/*/manifest.json") if ontology_id else base.glob("*/*/manifest.json")
    ...
    def _diff_release(self, previous: dict[str, Any] | None, contents: dict[str, str]) -> dict[str, Any]:
        ...
        for name, text in contents.items():
            if text.strip() and name != "shapes.ttl":
                for triple in _parse_rdf(text, name=name): current_graph.add(triple)
        previous_graph = Graph()
        if previous:
            release_dir = self.root / "published" / previous["ontology_id"] / previous["ontology_version"]
            for name in ("ontology.ttl", "skos.ttl"):
                text = (release_dir / name).read_text(encoding="utf-8")
                if text.strip():
                    for triple in _parse_rdf(text, name=name): previous_graph.add(triple)
        encode = lambda triple: [str(part) for part in triple]
        return {"added": sorted((encode(t) for t in set(current_graph) - set(previous_graph))), "removed": sorted((encode(t) for t in set(previous_graph) - set(current_graph)))}

    def _latest_release(self, ontology_id: str) -> dict[str, Any] | None:
        manifests = list((self.root / "published" / ontology_id).glob("*/manifest.json"))
        releases = [json.loads(path.read_text(encoding="utf-8")) for path in manifests]
        return max(releases, key=lambda item: item["published_at"]) if releases else None

    def _latest_review(self, draft_id: str) -> dict[str, Any]:
        reviews = self._read_jsonl(self._draft_dir(draft_id) / "reviews.jsonl")
        if not reviews: raise OntologyGovernanceError("draft must be validated before approval or waiver")
        return reviews[-1]

    def _draft_dir(self, draft_id: str) -> Path:
        if not draft_id.startswith("draft:") or any(char in draft_id for char in ("/", "\\", "..")):
    ...
    def _append_jsonl(path: Path, value: Any) -> None:
        ...
        with path.open("a", encoding="utf-8") as handle: handle.write(_canonical(value) + "\n")
    ...
    def _chain_append(self, path: Path, value: dict[str, Any]) -> None:
        ...
        value["integrity"] = {"algorithm": "sha256", "hash": _digest({key: item for key, item in value.items() if key != "integrity"})}
    ...
    def _verify_chain(self, path: Path) -> dict[str, Any]:
        ...
        for index, entry in enumerate(self._read_jsonl(path), start=1):
            ...
            if entry.get("previous_hash") != previous or entry.get("integrity", {}).get("hash") != _digest(payload):
                return {"valid": False, "entries_checked": index}
            previous = entry["integrity"]["hash"]
        return {"valid": True, "entries_checked": index if 'index' in locals() else 0, "head_hash": previous}
